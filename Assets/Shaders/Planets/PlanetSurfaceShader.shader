Shader "Cookie/PlanetSurfaceShader"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			bool intersectSphere(float3 C, float r, Ray R, inout float nearDistance, inout float farDistance)
			{
				float3 P = R.origin;
			...
			*/
			...
			void mainImage(out float4 fragColor, float3 dir, float3 org) {
				...
				if (hitBounds)
					fragColor.xyz = float3(0, 1, 1);
				else
					fragColor.xyz = float3(.5, 0, 0);
				fragColor.a = 1;
				// return ;
				...
				if (hitBounds) {
					...
					for (int i = 0; i < NUM_STEPS && (coverage < 1.0); ++i) {
						...
						X = eyeRay.origin + eyeRay.direction * (dt * float(i) + 1.);
